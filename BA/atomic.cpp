...
int atomicmeta::parse_card() {
    ...
    if(leftplaces[0] == 't')  //前半部分是token-count的形式
    {
        ...
        while (1) {
            ...
            index_t idex = cpn->getPPosition(placename);  //得到该库所的索引号
            if (idex == INDEX_ERROR) {
...
int atomicmeta::parse_fire() {
    ...
    while (1) {
        ...
        int idex = cpn->getTPosition(subs);  //变迁的序列号
        if(idex == INDEX_ERROR){
            return CONSISTENCY_ERROR;
        }
...
void atomictable::linkPlace2atomictable() {
    ...
    for (i = 0; i < cpn->placecount; i++) {
        cpn->place[i].atomicLinks.clear();  // clear last formula

        if (LTL_TYPE == PT_FIREABILITY) { //Firability
            int t_num = cpn->place[i].consumer.size();  // num of transition

            for (j = 0; j < t_num; j++) {
                index_t t_idx = cpn->place[i].consumer[j].idx;  // transition index
                ...
                for (k = 1; k <= atomiccount; k++) {
                    ...
                    if (it != atomics[k].fires.end()) {  // existing
                        cpn->place[i].atomicLinks.push_back(k);  // add to link table
            ...
            for (k = 0; k <= atomiccount; k++) { // check every atomicmeta
                ...
                if (atomics[k].leftexp.constnum == -1) { // if const num is not -1 then pass
                    ...
                    while (p) {
                        if (p->placeid == i) {
                            cpn->place[i].atomicLinks.push_back(k);
                            break;  // appear once
                        }
                        p = p->next;
                    }
                    if (!cpn->place[i].atomicLinks.empty())
        ...
        cpn->place[i].atomicLinks.shrink_to_fit();  // release memory
    }
}